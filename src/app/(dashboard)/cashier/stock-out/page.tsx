...
export default function CashierStockOutPage() {
  ...
  const handleTransactionSubmit = async (e: React.FormEvent) => {
    ...
    if (quantityNum > selectedStock.quantity) {
      return toast.error(lang === 'en' ? `Not enough stock! Only ${selectedStock.quantity} available.` : `በቂ ክምችት የለም! በክምችት ያለው ${selectedStock.quantity} ብቻ ነው።`);
    }
    ...
    try {
      ...
      // ==============================================================
      // FIX: AUTOMATED DOUBLE-ENTRY LOGIC NOW ADDED FOR CASHIERS
      // ==============================================================
      if (txType === 'TRANSFER_OUT') {
        const destinationStore = cashierStore === 'main' ? 'sub' : 'main';
        
        await Promise.all([
          // 1. Remove from Sender's Store
          ...
          // 2. Add to Receiver's Store Automatically
          stockService.createTransaction({
            type: 'TRANSFER_IN',
            storeId: destinationStore,
            items: [itemData],
            totalAmount,
            note: note || `Received from ${cashierStore === 'main' ? 'Bole' : 'Jemo'}`,
          })
        ]);
      ...
      setSearchQuery(''); setProductName(''); setSize(''); setColor(''); setItemQuantity(1); setItemUnitPrice(''); setNote(''); setTxType('SALE');
      fetchData();
    } catch (error: any) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 border-b border-[#E8E0D5]/60 dark:border-[#2D1E14]/40 pb-6">
        <div>
          ...
          <p className="text-sm text-[#6B4F3C] dark:text-[#A68F80] mt-1">
            {t.stockOut.subtitle || 'Sales and outgoing transfers'} · <span className="font-semibold text-[#A16438]">{getStoreLabel(cashierStore)}</span>
          </p>
        </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-[#160F0A] border border-[#E8E0D5] dark:border-[#2D1E14] rounded-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto shadow-2xl p-6 md:p-8 space-y-6 relative">
            <div className="flex items-start justify-between border-b border-[#E8E0D5]/60 dark:border-[#2D1E14]/40 pb-4">
              <div>
                <h3 className="text-xl font-bold font-serif text-[#3D2B1F] dark:text-[#FAF7F2] uppercase">{t.common.recordOutflowModal || 'Record Outflow'}</h3>
                <span className="text-xs text-[#6B4F3C] dark:text-[#A68F80] block mt-1 uppercase font-semibold">Locked Store: <span className="text-[#A16438]">{getStoreLabel(cashierStore)}</span></span>
              </div>
            ...
            <form onSubmit={handleTransactionSubmit} className="space-y-5">
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-wider text-[#3D2B1F] dark:text-[#FAF7F2] mb-2">{lang === 'en' ? 'Transaction Type' : 'የግብይት አይነት'}</label>
                <div className="flex items-center gap-2 bg-[#FAF7F2] dark:bg-[#0D0806] p-1.5 rounded-xl border border-[#E8E0D5] dark:border-[#2D1E14]">
                  <button type="button" onClick={() => setTxType('SALE')} className={`flex-1 text-[11px] font-bold uppercase tracking-wider py-2.5 rounded-lg transition-all ${txType === 'SALE' ? 'bg-[#A16438] text-white shadow-sm' : 'text-[#6B4F3C] dark:text-[#A68F80] hover:text-[#A16438]'}`}>{lang === 'en' ? 'Regular Sale' : 'መደበኛ ሽያጭ'}</button>
                  <button type="button" onClick={() => setTxType('TRANSFER_OUT')} className={`flex-1 text-[11px] font-bold uppercase tracking-wider py-2.5 rounded-lg transition-all ${txType === 'TRANSFER_OUT' ? 'bg-[#A16438] text-white shadow-sm' : 'text-[#6B4F3C] dark:text-[#A68F80] hover:text-[#A16438]'}`}>{lang === 'en' ? 'Transfer to other store' : 'ወደ ሌላ መደብር ዝውውር'}</button>
                </div>
              </div>
              ...
              <div className="relative">
                ...
                <input type="text" value={searchQuery} onChange={(e) => { setSearchQuery(e.target.value); setProductName(e.target.value); setIsDropdownOpen(true); }} onFocus={() => setIsDropdownOpen(true)} onBlur={() => setTimeout(() => setIsDropdownOpen(false), 200)} placeholder={lang === 'en' ? 'Type to search inventory...' : 'በክምችት ለመፈለግ ይጻፉ...'} className="w-full bg-[#FAF7F2] dark:bg-[#0D0806] border border-[#E8E0D5] dark:border-[#2D1E14] rounded-lg px-4 py-3 text-sm text-[#3D2B1F] dark:text-[#FAF7F2] focus:outline-none focus:border-[#A16438] focus:ring-1 focus:ring-[#A16438]" required />
                {isDropdownOpen && (
                  <ul className="absolute z-50 w-full mt-1 bg-white dark:bg-[#160F0A] border border-[#E8E0D5] dark:border-[#2D1E14] rounded-lg shadow-xl max-h-48 overflow-y-auto">
                    {filteredStock.length > 0 ? (
                      ...
                      <li className="px-4 py-3 text-sm text-center text-rose-500 italic">{lang === 'en' ? 'Product not found in stock.' : 'ምርቱ በክምችት ውስጥ የለም።'}</li>
                    )}
                  </ul>
                )}
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-[#160F0A] border border-[#E8E0D5] dark:border-[#2D1E14] rounded-2xl w-full max-w-5xl max-h-[90vh] overflow-y-auto shadow-2xl p-6 md:p-8 space-y-6">
            <div className="flex items-center justify-between border-b border-[#E8E0D5]/60 dark:border-[#2D1E14]/40 pb-4">
              <div>
                <h3 className="text-xl font-bold font-serif text-[#3D2B1F] dark:text-[#FAF7F2] uppercase">{lang === 'en' ? 'Daily Report' : 'የዕለት ሪፖርት'}</h3>
                ...
                <tbody className="divide-y divide-[#E8E0D5]/40 dark:divide-[#2D1E14]/30">
                  {selectedDateTransactions.map((tx) => (
                    <tr key={tx.id} className="text-sm text-[#3D2B1F] dark:text-[#FAF7F2]/90 hover:bg-[#FAF7F2]/50 dark:hover:bg-[#0D0806]/30 transition-colors">
                      ...
                      <td className="py-4 text-center">
                        <button onClick={() => handleDelete(tx.id)} className="p-1.5 rounded-lg border border-[#E8E0D5] dark:border-[#2D1E14] text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/20 transition-colors" title="Delete">🗑️</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
