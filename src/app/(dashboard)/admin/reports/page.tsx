...
export default function AdminReportsPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.error('Error fetching backend data for reports:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const groupedByDay = filteredTransactions.reduce((acc: any, tx) => {
    ...
    tx.items.forEach((it) => {
      if (['PURCHASE', 'TRANSFER_IN', 'RETURN_IN', 'STOCK_IN'].includes(tx.type)) {
        acc[day].totalStockInQty += it.quantity;
      } else if (['SALE', 'TRANSFER_OUT', 'DAMAGE', 'RETURN_TO_SUPPLIER'].includes(tx.type)) {
  ...
  const getDetailedDayReport = (targetDateKey: string): DailyStockItemReport[] => {
    const itemGroups: { [key: string]: { storeId: 'main' | 'sub'; productName: string; size: string; color: string } } = {};

    // Seed current products
    products.forEach((p) => {
      const colorLabel = p.color || '-';
      const key = `${p.storeId}-${p.name}-${p.size}-${colorLabel}`;
      itemGroups[key] = {
        storeId: p.storeId,
        productName: p.name,
        size: p.size || '-',
    ...
    // Seed historical transactions items
    transactions.forEach((tx) => {
      tx.items.forEach((it) => {
        const colorLabel = it.color || '-';
        const key = `${tx.storeId}-${it.productName}-${it.size}-${colorLabel}`;
        if (!itemGroups[key]) {
          itemGroups[key] = {
            storeId: tx.storeId,
    ...
    Object.values(itemGroups).forEach((group) => {
      if (storeFilter !== 'all' && group.storeId !== storeFilter) return;
      ...
      transactions.forEach((tx) => {
        if (tx.storeId !== group.storeId) return;
      ...
      const currentQty = products
        .filter(
          (p) =>
            p.storeId === group.storeId &&
            p.name === group.productName &&
            (p.size || '-') === group.size &&
            (p.color || '-') === group.color
        )
  ...
  const monthlySales = transactions
    .filter((tx) => {
      ...
      return (
        ...
        (storeFilter === 'all' || tx.storeId === storeFilter)
      );
    })
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 border-b border-[#E8E0D5]/60 dark:border-[#2D1E14]/40 pb-6">
        <div>
          ...
          <p className="text-sm text-[#6B4F3C] dark:text-[#A68F80] mt-1">
            {t.reports.subtitle || 'Audited daily inventory logs'} <span className="font-semibold text-[#A16438]">(Admin View)</span>
          </p>
        </div>

        {/* Store filter */}
        <div className="flex items-center self-start bg-white/40 dark:bg-black/20 border border-[#E8E0D5] dark:border-[#2D1E14] rounded-full p-1">
          <button
            onClick={() => setStoreFilter('all')}
            className={`text-xs font-bold px-4 py-2 rounded-full transition-all uppercase tracking-wider ${
              storeFilter === 'all'
                ? 'bg-[#A16438] text-white shadow-sm'
                : 'text-[#6B4F3C] dark:text-[#A68F80] hover:text-[#A16438]'
            }`}
          >
            {t.inventory.bothBranches || 'All'}
          </button>
          <button
          ...
          >
            {t.inventory.mainStoreFilter || 'Bole'}
          </button>
          <button
            onClick={() => setStoreFilter('sub')}
            className={`text-xs font-bold px-4 py-2 rounded-full transition-all uppercase tracking-wider ${
              storeFilter === 'sub'
                ? 'bg-[#A16438] text-white shadow-sm'
                : 'text-[#6B4F3C] dark:text-[#A68F80] hover:text-[#A16438]'
                  ...
                  <tbody className="divide-y divide-[#E8E0D5]/40 dark:divide-[#2D1E14]/30">
                    {activeReportData.map((row, idx) => (
                      <tr key={idx} className="text-sm text-[#3D2B1F] dark:text-[#FAF7F2]/90 hover:bg-[#FAF7F2]/50 dark:hover:bg-[#0D0806]/30 transition-colors">
                        <td className="py-4 font-medium text-[#6B4F3C] dark:text-[#A68F80] uppercase tracking-wider text-xs">
